...
эти запросы к backend. Для этой схемы используйте `VITE_API_BASE_URL=/api`, а
внутреннюю цель Vite proxy при необходимости переопределяйте через
`KORNIX_DEV_API_PROXY_TARGET`, например `http://host.docker.internal:8001`.
...
Проверка production bundle должна падать, если в JS попал прямой локальный
backend URL:
...
Standalone nginx-контейнер подходит для локальной production-like проверки,
когда backend опубликован на host-порту `8001`. В VDS/production он не должен
заменять unified reverse proxy: backend production compose не публикует `8001`
наружу, а API-трафик должен идти через Caddy внутри общей Docker-сети на
`app:8000`.
...
backend через `host.docker.internal:8001`; в VDS-контуре `/api/*` обслуживает
Caddy и проксирует запросы к `app:8000`.
