...
- Диапазон регулирования влагозапасов по умолчанию приведён к операторскому
  контракту `0.75..0.90 НВ`, чтобы карта, график и карточка поля считали полив
  от одной нижней границы.
- Production compose, release checklist и smoke-документация синхронизированы с
  VDS-контрактом `/api`, canonical frontend URL и backend proxy через
  `host.docker.internal:8001`.
- Dev env templates, Vite proxy config и frontend contract-check синхронизированы
  с same-origin `/api`: browser API base больше не смешивается с backend proxy
  upstream `KORNIX_DEV_API_PROXY_TARGET`.
- Production nginx smoke стал воспроизводимым после pull: ручной `docker run`
  получает `host.docker.internal`, а smoke-скрипт ждёт `/healthz`, проверяет
  статусы SPA routes, `404` для отсутствующего asset и CSP headers.
- `/healthz` в production nginx больше не отдаёт дублирующийся `Content-Type`;
  тип ответа задаётся через `default_type text/plain`.
- Smoke-проверка frontend editable approval больше не падает на актуальном
  `nginx.conf`, где `/api/` проксируется через переменную upstream.
- Календарный блок ввода поливов опущен на `4px` за счёт верхней белой полосы,
  чтобы визуально точнее совместить строки календаря со списком полей.
- Вертикальная прокрутка календаря ввода поливов теперь зажимается общей
  нижней границей со списком полей, чтобы строки не расходились внизу.
- Название метрики `days_after_sowing` на frontend изменено с "Дней после сева"
  на "Дней после посадки".
- В компактной карточке карты строки текущего дня окрашены тёмно-синим, а
  прогнозные строки - тёмно-оранжевым.
- В строке суммарного испарения и осадков компактной карточки карты теперь
  показывается недельный дефицит со знаком как разность испарения и осадков.
- Компактная карточка поля на карте теперь показывает рекомендованный полив
  за неделю до нижней/верхней границы регулирования, рассчитанный по
  прогнозному влагозапасу на конечную дату прогноза.
- Компактная карточка поля на карте теперь показывает дату `today+7`
  с пометкой "прогноз на семь дней", а также суммарное испарение и осадки
  за семь следующих дней.
- Полив до нижней/верхней границы в компактной карточке карты теперь считается
  от пользовательского диапазона регулирования графика, а не от backend
  `koef_lower_limit`/`koef_upper_limit`.
- Обновление страницы больше не откатывает выбранный день карты через переход
  на вкладку "Карта", а компактная карточка поля достраивается на frontend даже
  при старом `popupSummary` от backend.
- Компактная карточка поля на карте теперь показывает полив до нижней и
  верхней границ регулирования.
- Компактная карточка поля на карте теперь показывает влажность
  корнеобитаемого слоя в процентах от НВ.
- Даты в компактной карточке поля на карте теперь выводятся в формате
  `DD.MM.YY`.
- Подписи полей на карте теперь скрывают tenant-префикс `SP:` и показывают
  только рабочий номер поля.
...
### Changed
- На графике влагозапасов зона между верхним пределом регулирования и НВ
  теперь залита бледно-голубым цветом вместо бледно-жёлтого.
- В легенде карты и кнопках полей зоны влагозапасов разделены на
  "Влажно" (бледно-голубой) и "Сухо" (бледно-жёлтый), включая подсказки
  текущего дня и прогноза.
...
- Y-шкалы графиков водного режима больше не строят линии сетки и подписи по
  дробным значениям.
- Диапазон управления влагозапасами считается в долях НВ с текущим default
  `0.75—0.90`, ВЗ выводится отдельной оранжевой линией, а текущие влагозапасы
...
- В правую панель графика добавлены пользовательские настройки диапазона
  регулирования в долях НВ с шагом `0.01`, значениями по умолчанию `0.75—0.90`
...
- Hover-подсказки цветных карточек полей приведены к цвету зоны влагозапасов:
  голубая — `Влажно`, зелёная — `Влагозапасы в норме`, жёлтая — `Сухо`,
  розовая — `Нужен полив`.
...
- Диапазон управления на графике влагозапасов по умолчанию строится от
  `0.75 НВ` до `0.9 НВ`, при этом линии НВ и ВЗ берутся из отдельных
  backend-метрик.
...
- Локальный integration-dev профиль держит browser-facing `VITE_API_BASE_URL=/api`,
  а backend upstream задаёт отдельным `KORNIX_DEV_API_PROXY_TARGET`, чтобы
  frontend smoke не зависел от CORS и не встраивал прямой backend origin в bundle.
